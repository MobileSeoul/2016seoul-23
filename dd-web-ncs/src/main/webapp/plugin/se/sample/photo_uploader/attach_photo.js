    ...
    function html5Upload() {	
    	...
    	sUploadURL= '/plugin/se/sample/photo_uploader/file_uploader_html5.jsp'; 	//upload URL
			...
			onload : function(res){ // 요청이 완료되면 실행될 콜백 함수
				...
				if (res.readyState() == 4) {
					if(sResString.indexOf("NOTALLOW_") > -1){
						...
						alert("test 이미지 파일(jpg,gif,png,bmp)만 업로드 하실 수 있습니다. ("+sFileName+")");
					}else{
	...
	function onAjaxError (){
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 			sUrl  : '/plugin/se/sample/photo_uploader/file_uploader.jsp',	//샘플 URL입니다.
 			sCallback : '/plugin/se/sample/photo_uploader/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
