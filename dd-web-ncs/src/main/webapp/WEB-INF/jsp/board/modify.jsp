...
<div id="wrap" class="write">
	<header>
		...
		<h1 class="tit"><em>글수정</em></h1>
	</header>

<form name="frm" method="post">
	<input type="hidden" name="ub_seq" value="${board.UB_SEQ }" />
	<input type="text" name="refererUrl" value="${params.refererUrl }" />
	<section id="container">
		<div class="form-area">
			<ul class="form-list">
				<li><label for="ub_title">제목<sup></sup></label><span><input type="text" id="ub_title" name="ub_title" value="${board.UB_TITLE }" class="full"></span></li>
				<li><label for="location">위치</label>
					<span class="two-select">
						<select title="시 선택" id="ub_area1" name="ub_area1">
							<option>서울시</option>
						</select>
<c:set var="AREA2" value="${board.UB_AREA2}" />						
...
<c:forEach var="item" items="${cd:getCodeList(params.CODE, 'MC_CATE_CODE')}" varStatus="status">
	...
	<c:forEach var="cate" items="${categoryList}" varStatus="status1">
		...
		<c:if test="${item.CD eq board.UB_CATEGORY}">
			<c:set var="SELECTED" value="Y" />
				...
				<li><label for="ub_cmt">내용</label><span><textarea rows="8" id="ub_cmt" name="ub_cmt" class="full">${board.UB_CMT}</textarea></span></li>
			</ul>
		</div>

		<div class="btn-area bottom">
			<button type="button" class="btn-blue" data-type="save"><em>수정</em></button>
		</div>
	</section>
	...
	$(document).on("click","[data-type='save']",function(){
		...
		$.ajax({
			...
			success : function(result) {
				var refer = $("input[name='refererUrl']").val()
				if(refer == ""){
					refer = "/main/main.do";
				}
				if(result.data){
					alertLayer("수정되었습니다." ,refer ,"");
				}else{
